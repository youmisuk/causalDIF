...
#clusterExport(cluster, varlist = c("DGP_D1", "DIFmodel", "reps", "design_value"))
...
foreach(bb = 1:reps, .packages = c('mgcv','nnet','foreach','doParallel','difR', 'caret', 'ltm', "mirt"), .combine = 'rbind') %dopar% {
  ...
  for (d in 1:4) {
    ...
    repeat{  
      itemlist <- DGP_D1(N = 2000, J = J, focal.frac = 0.5, DIFmag = design_value[[d]][1], itemImpact=design_value[[d]][2])
    ...
    CDIF_rlst <- DIFmodel(itemdata=item_data_pilot, group = itemlist$group, theta =  itemlist$theta)  ##  
    ...
    CDIF_rlst_est <- DIFmodel(itemdata=item_data_pilot, group = itemlist$group, theta =  theta.est)  ##  
    
    temp.est <- as.numeric(c(CDIF_rlst[(J+1):tJ,1], Std_rlst$PDIF[(J+1):tJ], MH_rlst$p.value[(J+1):tJ], Logistic_rlst$p.value[(J+1):tJ],
