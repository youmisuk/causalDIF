...
for (d in 1:4) {
    ...
    repeat{  
      itemlist <- DGP_D2(N = 2000, J = J, focal.frac = 0.5, DIFmag = design_value[[d]][1], itemImpact=design_value[[d]][2])
      ...
      if (design_value[[d]][2] == FALSE) {
        ...
        model_2pl <- mirt(data = item_data_theta, itemtype = "2PL", verbose = F, technical = list(NCYCLES = 500))
        theta.est <- fscores(model_2pl, itemtype = "2PL", response.pattern = item_data_theta, method="EAP")[, "F1"]
        
      } else {
        
        model_mg <- multipleGroup(item_data_theta, 1, group = as.factor(itemlist$group), itemtype = "2PL", optimizer = 'NR',
    ...
    CDIF_rlst <- DIFmodel(itemdata=item_data_pilot, group = itemlist$group, theta =  itemlist$theta, confounder=data.frame(itemlist$X))  ##  
    ...
    CDIF_rlst_est <- DIFmodel(itemdata=item_data_pilot, group = itemlist$group, theta =  theta.est, confounder=data.frame(itemlist$X))  ##  

    temp.est <- as.numeric(c(CDIF=CDIF_rlst[(J+1):tJ,1], Std=Std_rlst$PDIF[(J+1):tJ], MH=MH_rlst$p.value[(J+1):tJ], LR=Logistic_rlst$p.value[(J+1):tJ],
                        CDIF_est=CDIF_rlst_est[(J+1):tJ,1], Std_sum=Std_rlst_sum$PDIF[(J+1):tJ], MH_sum=MH_rlst_sum$p.value[(J+1):tJ], LR_sum=Logistic_rlst_sum$p.value[(J+1):tJ]))
...
rlst_D1 <- cbind.data.frame(RegAdj=apply(simu_results[,1:30] < 0.05, 2, mean),
                             Std=apply(abs(simu_results[,31:60]) > 0.05, 2, mean),
                             MH=apply(simu_results[,61:90] < 0.05, 2, mean),
                             Logistic=apply(simu_results[,91:120] < 0.05, 2, mean))

# Scenario 1 with estimated ability
rlst_D1_sum <- cbind.data.frame(RegAdj=apply(simu_results[, 121:150] < 0.05, 2, mean),
                                Std=apply(abs(simu_results[,151:180]) > 0.05, 2, mean),
                                MH=apply(simu_results[,181:210] < 0.05, 2, mean),
                                Logistic=apply(simu_results[,211:240] < 0.05, 2, mean))

# Scenario 2 with true ability
rlst_D2 <- cbind.data.frame(RegAdj=apply(simu_results[,241:270] < 0.05, 2, mean),
                             Std=apply(abs(simu_results[,271:300]) > 0.05, 2, mean),
                             MH=apply(simu_results[,301:330] < 0.05, 2, mean),
                             Logistic=apply(simu_results[,331:360] < 0.05, 2, mean))

# Scenario 2 with estimated ability
rlst_D2_sum <- cbind.data.frame(RegAdj=apply(simu_results[,361:390 ] < 0.05, 2, mean),
                                Std=apply(abs(simu_results[,391:420]) > 0.05, 2, mean),
                                MH=apply(simu_results[,421:450] < 0.05, 2, mean),
                                Logistic=apply(simu_results[, 451:480] < 0.05, 2, mean))

# Scenario 3 with true ability
rlst_D3 <- cbind.data.frame(RegAdj=apply(simu_results[,481:510] < 0.05, 2, mean),
...
rlst_D3_sum <- cbind.data.frame(RegAdj=apply(simu_results[, 601:630] < 0.05, 2, mean),
                                Std=apply(abs(simu_results[,631:660]) > 0.05, 2, mean),
                                MH=apply(simu_results[,661:690] < 0.05, 2, mean),
                                Logistic=apply(simu_results[,691:720] < 0.05, 2, mean))

# Scenario 4 with true ability
rlst_D4 <- cbind.data.frame(RegAdj=apply(simu_results[,  721:750] < 0.05, 2, mean),
                             Std=apply(abs(simu_results[,  751:780]) > 0.05, 2, mean),
                             MH=apply(simu_results[,  781:810] < 0.05, 2, mean),
                             Logistic=apply(simu_results[, 811:840] < 0.05, 2, mean))

# Scenario 14 with estimated ability
rlst_D4_sum <- cbind.data.frame(RegAdj=apply(simu_results[,841:870] < 0.05, 2, mean),
                                Std=apply(abs(simu_results[,871:900]) > 0.05, 2, mean),
                                MH=apply(simu_results[,901:930] < 0.05, 2, mean),
                                Logistic=apply(simu_results[,931:960] < 0.05, 2, mean))
